...
fn a_missing_file_is_an_empty_set_rather_than_an_error() {
    ...
    assert_eq!(
        script.options_for(Path::new("anything.wav"), &dir),
        Options::default()
    );
}
...
fn paths_match_however_they_are_spelt() {
    ...
    assert!(
        script.has_entry("UI\\Click.wav"),
        "a Windows path names the same sound"
    );
}
...
fn a_nonsense_gain_falls_back_instead_of_compiling_silence() {
    ...
    for bad in ["0", "-1.5", "banana", ""] {
        let script =
            Script::parse(&format!(r#"sound {{ "file" "a.wav" "gain" "{bad}" }}"#)).unwrap();
...
fn an_unknown_encoding_falls_back_rather_than_failing_the_build() {
    ...
    assert_eq!(
        script
            .options_for(Path::new("a.wav"), Path::new("."))
            .encoding,
        Encoding::Adpcm
    );
}
...
fn an_explicit_zero_loop_means_do_not_loop_rather_than_say_nothing() {
    ...
    // itself declares".
    let script =
        Script::parse(r#"sound { "file" "a.wav" "loopstart" "0" "loopend" "0" }"#).unwrap();
    let options = script.options_for(Path::new("a.wav"), Path::new("."));
...
fn a_written_script_only_states_what_differs_from_the_defaults() {
    ...
    assert!(
        !plain.contains("gain"),
        "plain entry should be bare: {plain}"
    );
}
...
fn clearing_an_entry_puts_it_back_on_the_defaults() {
    ...
    assert_eq!(
        script.options_for(Path::new("a.wav"), Path::new(".")).gain,
        2.0
    );
