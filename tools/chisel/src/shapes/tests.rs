...
fn all_valid(solids: &[Solid]) {
    assert!(
        !solids.is_empty(),
        "a shape that produces nothing is a tool that does nothing"
    );
    ...
    for shape in Shape::all() {
        for axis in 0..3 {
            ...
            for solid in build(shape, bounds, axis, Options::default(), "dev/grid") {
                ...
                for i in 0..3 {
                    assert!(
                        got.min[i] >= bounds.min[i] - 0.01,
                        "{} escaped: {got:?}",
                        shape.label()
                    );
                    assert!(
                        got.max[i] <= bounds.max[i] + 0.01,
                        "{} escaped: {got:?}",
                        shape.label()
                    );
                }
...
fn a_box_too_flat_to_hold_anything_produces_nothing_rather_than_rubbish() {
    ...
    for shape in Shape::all() {
        assert!(
            build(shape, flat, 2, Options::default(), "dev/grid").is_empty(),
            "{}",
            shape.label()
        );
    }
...
fn a_wedge_is_a_ramp_rather_than_a_triangular_wall_standing_on_end() {
    ...
    assert!(
        wedge.contains_point(Vec3::new(200.0, 8.0, 32.0)),
        "solid at one side"
    );
    assert!(
        wedge.contains_point(Vec3::new(200.0, 248.0, 32.0)),
        "and at the other"
    );

    // ...and it climbs: low down it reaches back, high up it does not.
    assert!(
        wedge.contains_point(Vec3::new(140.0, 128.0, 8.0)),
        "the floor reaches the middle"
    );
    assert!(
        !wedge.contains_point(Vec3::new(140.0, 128.0, 240.0)),
...
fn a_wedge_slopes_the_right_way_whichever_pane_it_was_drawn_in() {
    for axis in 0..3 {
        ...
        low[rise] = 16.0;
        assert!(
            wedge.contains_point(high),
            "axis {axis}: the high end is solid at the top"
        );
        assert!(
            !wedge.contains_point(low),
            "axis {axis}: the low end is not"
        );
    }
}
...
fn a_cylinder_is_hollow_nowhere_and_round_enough_to_be_worth_it() {
    ...
    assert!(
        pillar.contains_point(Vec3::new(128.0, 128.0, 128.0)),
        "solid down the middle"
    );
    // The corners of the box are outside a shape inscribed in it.
    assert!(
        !pillar.contains_point(Vec3::new(4.0, 4.0, 128.0)),
        "the box corner is cut off"
    );
}
...
fn a_cone_narrows_to_its_apex() {
    ...
    let cone = &solids[0];
    assert!(
        cone.contains_point(Vec3::new(128.0, 128.0, 8.0)),
        "wide at the base"
    );
    assert!(
        !cone.contains_point(Vec3::new(30.0, 30.0, 240.0)),
...
fn an_arch_has_a_hole_in_it() {
    ...
    assert!(
        solids.iter().any(|s| s.contains_point(on_the_ring)),
        "the ring is solid"
    );
}
...
fn an_arch_wall_thicker_than_the_arch_still_leaves_a_hole() {
    ...
    assert!(
        solids
            .iter()
            .all(|s| !s.contains_point(Vec3::new(128.0, 128.0, 128.0)))
    );
}
...
fn a_half_arch_is_the_top_half_because_that_is_the_bit_you_walk_under() {
    ...
    for axis in 0..3 {
        ...
        assert!(
            solids.iter().all(|s| !s.contains_point(under)),
            "axis {axis}: the doorway is blocked"
        );
    }
}
...
fn stairs_are_a_brush_per_step_and_each_one_reaches_the_floor() {
    ...
    for pair in heights.windows(2) {
        assert!(
            pair[1] > pair[0],
            "each step is taller than the last: {heights:?}"
        );
    }
}
...
fn stairs_run_along_the_longer_side_of_the_box_they_are_drawn_in() {
    ...
    assert!(
        ...
        "the tread is narrow along the run: {first:?}"
    );
}
...
fn a_shape_carries_the_material_it_was_asked_for() {
    for shape in Shape::all() {
        for solid in build(shape, box_(), 2, Options::default(), "tools/clip") {
            assert!(
                solid.sides.iter().all(|s| s.material == "tools/clip"),
                "{}",
                shape.label()
            );
        }
...
fn a_shape_swept_along_x_lies_on_its_side() {
    ...
    assert!(
        bounds.size().y < 256.0,
        "and inscribed across it: {bounds:?}"
    );
}
...
fn only_the_shapes_that_use_a_setting_say_they_do() {
    ...
    assert!(
        Shape::Stairs.uses_sides(),
        "the step count is the side count"
    );
}
