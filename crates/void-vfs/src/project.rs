...
//! `.voidproj` -- a project's own account of where its content is.
//!
...
pub const EXTENSION: &str = "voidproj";
...
impl Project {
    ...
    pub fn write_new(path: &Path, name: &str, content_relative: &str) -> anyhow::Result<()> {
        ...
        let body = format!(
            "// A VoidEngine project. Every tool reads this to find the content\n\
