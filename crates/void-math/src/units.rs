...
//! The units everything in VoidEngine is measured in.
//!
//! Distances are **void units** (`vu`). One void unit is one inch, which fixes
//! the scale of everything else: a player is 72 vu tall, walks at 320 vu/s,
//! and a comfortable corridor is about 128 vu wide. The choice is inherited
...
//! grid of 16 vu gives doorways and stair risers that are already right.
//!
...
//! the editor made up. Written as `128 vu (3.25 m)` it can be argued with.
//!
//! | Quantity | Unit | Symbol |
//! |---|---|---|
//! | Distance | void unit | `vu` |
//! | Area | square void unit | `vu²` |
//! | Volume | cubic void unit | `vu³` |
//! | Speed | void units per second | `vu/s` |
//! | Angle | degree | `°` |
//! | Time | second | `s` |

/// Void units in one metre.
///
/// A void unit is an inch, so this is the inches-per-metre conversion and not
/// a number anyone gets to choose.
pub const VU_PER_METRE: f32 = 39.3701;

/// Void units in one foot.
...
pub fn metres(vu: f32) -> f32 { vu / VU_PER_METRE }
pub fn from_metres(m: f32) -> f32 { m * VU_PER_METRE }
pub fn feet(vu: f32) -> f32 { vu / VU_PER_FOOT }
...
pub fn length(vu: f32) -> String {
    format!("{} vu ({:.2} m)", trim(vu), metres(vu))
}

/// Format a distance with just the unit, for places too narrow for both.
pub fn length_short(vu: f32) -> String {
    format!("{} vu", trim(vu))
}
...
pub fn area(vu2: f32) -> String {
    format!("{} vu\u{b2}", trim(vu2))
}

pub fn volume(vu3: f32) -> String {
    format!("{} vu\u{b3}", trim(vu3))
}

pub fn speed(vu_per_second: f32) -> String {
    format!("{} vu/s ({:.1} m/s)", trim(vu_per_second), metres(vu_per_second))
}

/// How tall something is in players, for judging a space.
pub fn in_players(vu: f32) -> String {
    format!("{:.1}x player", vu / PLAYER_HEIGHT)
}
...
mod tests {
    ...
    fn distances_carry_their_unit_and_a_metric_equivalent() {
        assert_eq!(length(128.0), "128 vu (3.25 m)");
        assert_eq!(length_short(128.0), "128 vu");
        assert_eq!(length(0.5), "0.5 vu (0.01 m)");
    }

    #[test]
    fn sizes_read_the_way_a_brush_is_measured() {
        assert_eq!(size(64.0, 128.0, 16.0), "64 x 128 x 16 vu (1.63 x 3.25 x 0.41 m)");
    }

    #[test]
    fn speeds_and_areas_have_units_too() {
        assert_eq!(speed(320.0), "320 vu/s (8.1 m/s)");
        assert_eq!(area(1024.0), "1024 vu\u{b2}");
        assert_eq!(volume(4096.0), "4096 vu\u{b3}");
    }
    ...
    fn whole_numbers_do_not_grow_decimal_places() {
        assert_eq!(length_short(16.0), "16 vu");
        assert_eq!(length_short(-0.25), "-0.25 vu");
    }
}
