...
//!
//! `content/voidengine.voiddef` is what Chisel shows in its property
...
use void_entity::{ClassKind, Schema};

fn schema() -> Schema {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../../content/voidengine.voiddef");
...
fn every_registered_class_is_described() {
    let schema = schema();
    let registry = void_game::registry();
    ...
    assert!(
        missing.is_empty(),
        "these classes exist in the game but not in content/voidengine.voiddef, \
