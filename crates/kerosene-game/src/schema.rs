...
// them by heart.
//
// It is checked against the game's class registry by a test in kerosene-game, so
...
class
{
    ...
    key {
        "name" "skyname" "label" "Sky material" "type" "material" "default" "dev/sky_kero"
...
class
{
    ...
    key {
        ...
        choice { "value" "1" "label" "Start turning" }
        choice { "value" "2" "label" "Turn about the forward axis" }
        choice { "value" "4" "label" "Turn about the left axis" }
    }
    key { "name" "maxspeed" "label" "Speed" "type" "float" "default" "100"
          "help" "Degrees per second. Negative turns the other way." }
    input { "name" "Start" }
    input { "name" "Stop" }
    input { "name" "Toggle" }
    input { "name" "Reverse" "help" "Turn the other way, at the same speed." }
    input { "name" "SetSpeed" "parameter" "degrees per second" }
}
...
class
{
    ...
    key { "name" "radius" "label" "Audible radius" "type" "float" "default" "0"
          "help" "How far it carries, in kerosene units. 0 uses whatever the sound script says." }
...
class
{
    ...
    input  { "name" "Play" "help" "Play it, once." }
