...
//! server all take the identical path. VoidEngine keeps that, because the
...
//!
//! ```
//! # use void_console::{Console, ConVarFlags};
...
mod tests {
    ...
    fn non_numeric_convars_keep_their_text() {
        let mut c = Console::new();
        c.register_cvar("map", "void_start", ConVarFlags::NONE, "");
        c.execute("map void_arena");
        assert_eq!(c.string("map"), "void_arena");
    }
