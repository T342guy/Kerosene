...
fn a_definition_falls_back_to_sensible_values() {
    ...
    assert_eq!(def.file, "sound/ambient/hum.voidaud");
}
...
fn a_name_nobody_defined_is_taken_as_a_path() {
    ...
    assert_eq!(bank.resolve("test").0, "sound/test.voidaud");
...
fn a_bare_name_looks_for_the_compiled_form_first() {
    ...
    assert_eq!(candidates[0], "sound/ui/click.voidaud");
...
fn a_name_the_script_defines_resolves_through_the_file_it_names() {
    ...
    assert_eq!(candidates[0], "sound/door/move.voidaud");
...
fn a_flac_beside_the_name_is_found_for_the_message() {
    ...
    assert_eq!(
        uncompiled_source("sound/ambient/track.voidaud", there).as_deref(),
...
fn nothing_beside_the_name_means_nothing_to_say_about_it() {
    assert_eq!(uncompiled_source("sound/ambient/track.voidaud", |_| false), None);
}
