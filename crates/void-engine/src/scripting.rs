...
//!
//! `void-script` deliberately knows nothing about the engine: it reads a
...
use void_entity::{EntityId, Target};
use void_script::{EntityView, ScriptAction, ScriptLevel, WorldView};

/// Where a map's script lives, given the map's name.
pub fn script_path(name: &str) -> String {
    format!("scripts/{name}.{}", void_script::EXTENSION)
}
...
/// [`void_entity::EntityWorld::exists`].
...
impl Engine {
    ...
    /// for.
    pub fn run_script(&mut self, source: &str) -> Result<Option<String>, void_script::ScriptError> {
        ...
        for action in actions {
            match action {
                ...
                ScriptAction::FireInput { target, input, parameter, delay } => {
                    let target = match void_script::parse_id_target(&target) {
                ...
                ScriptAction::SetField { entity, key, value } => {
                    ...
                    if let Some(e) = self.entities.get_mut(id) {
                        e.fields.set(&key, void_entity::Value::Text(value));
                    }
                }
                ScriptAction::SetOrigin { entity, origin } => {
                    ...
                    if let Some(e) = self.entities.get_mut(id) {
                        e.origin = origin;
                        e.fields.set("origin", void_entity::Value::Vector(origin));
                    }
                }
    ...
    pub fn take_entity_requests(&mut self) {
        for request in self.entities.take_requests() {
            ...
            match request.kind.as_str() {
                ...
                void_entity::host_requests::SCRIPT_FILE => {
...
fn entity_view(entity: &void_entity::Entity) -> EntityView {
